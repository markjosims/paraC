...
Patterns are YAML dictionaries that consist of two keys, 'pattern' and '_ref'.
...
The '_ref' attribute, as is the case for classes in [Inventory](config/inventory/README.md) objects, is a unique string that can be used to refer to the pattern.
Like `Inventory` classes, the '_ref' string must be surrounded by angle braces.
```yaml
kind: Patterns
patterns:
    - non_high_vowel:
    ...
    - voiced_stop:
        ...
        _ref: <VcdStop>
```
Patterns can be used to build other patterns by using the '_ref' value:
```yaml
kind: Patterns
pattern:
